...
async function getMovieListByActorName(name){
    const actorRes = await rp(`https://api.themoviedb.org/3/search/person?api_key=5e91bf33609f91a05655f2c9aa0ff7ae&query=` + encodeURIComponent(name));
    let actorJson = JSON.parse(actorRes);
    let actorId = actorJson.results[0].id;

    const response = await rp(`https://api.themoviedb.org/3/person/${actorId}/movie_credits?api_key=5e91bf33609f91a05655f2c9aa0ff7ae`);
...
for(const movieId of movieIds){
    let response = await rp(`https://api.themoviedb.org/3/movie/${movieId}?api_key=5e91bf33609f91a05655f2c9aa0ff7ae`);
...
async function getRandomMovie(){
    ...
    let movieRes = await rp(`https://api.themoviedb.org/3/discover/movie?api_key=5e91bf33609f91a05655f2c9aa0ff7ae&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&primary_release_year=${randomYear}`);
